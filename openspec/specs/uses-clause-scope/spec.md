...
Extrair a cláusula `uses` de um source Pascal reconhecendo explicitamente
as seções `interface` e `implementation`, permitindo escolher se o
resultado inclui só a `interface` ou as duas seções combinadas — a base
de parsing para o Uses Clause Scope descrito no `CONTEXT.md`.
